...
class Iter_BCD:
    ...
    def __init__(self, ppc, active_set_data, T_delta, union_analysis=None, use_theta=True, use_zeta=True):
        ...
        self.gamma_base = 1.0   # gamma 缩放基准
    ...
    def initialize_theta_values(self, union_analysis=None):
        ...
        for constraint in union_constraints:
            ...
            # 为每个相关机组创建3个theta变量的零值 (theta_0, theta_1, theta_2)
    ...
    def add_theta_variables_for_branches(self, union_analysis=None):
        ...
        print(f"为 {len(union_constraints)} 个约束创建theta变量")
        ...
        for constraint in union_constraints:
            ...
            # 为每个相关机组创建3个theta变量 (theta_0, theta_1, theta_2)
            for coeff_info in nonzero_coefficients:
                unit_id = coeff_info['unit_id']
                for order in range(3):
                    var_name = f'theta_branch_{branch_id}_unit_{unit_id}_time_{time_slot}_{order}'
...
### zeta ###
    ...
    def _add_parametric_balance_power_penalties_pg_block(self, model, x, sample_id,  zeta_values=None, union_analysis=None):
        ...
        try:
            ...
            for constraint in union_constraints:
                ...
                model.addConstr(parametric_rhs_viol >= - lhs_expr + parametric_rhs, name=f'parametric_balance_power_lower_rhs_viol_{unit_id}_{time_slot}')
                model.addConstr(parametric_rhs_abs >= lhs_expr - parametric_rhs, name=f'parametric_balance_power_lower_rhs_abs1_{unit_id}_{time_slot}')
                model.addConstr(parametric_rhs_abs >= -lhs_expr + parametric_rhs, name=f'parametric_balance_power_lower_rhs_abs2_{unit_id}_{time_slot}')
                
                obj_primal += parametric_rhs_viol

                obj_opt += parametric_rhs_abs * self.ita_lower[sample_id, 0, time_slot]
                ...
                model.addConstr(parametric_rhs_viol >= lhs_expr - parametric_rhs, name=f'parametric_balance_power_upper_rhs_viol_{unit_id}_{time_slot}')
                model.addConstr(parametric_rhs_abs >= lhs_expr - parametric_rhs, name=f'parametric_balance_power_upper_rhs_abs1_{unit_id}_{time_slot}')
                model.addConstr(parametric_rhs_abs >= -lhs_expr + parametric_rhs, name=f'parametric_balance_power_upper_rhs_abs2_{unit_id}_{time_slot}')

                obj_primal += parametric_rhs_viol

                obj_opt += parametric_rhs_abs * self.ita_upper[sample_id, 0, time_slot]
    ...
    def _add_variational_balance_power_primal_zeta_block(self, model, x, sample_id, zeta, union_analysis=None):
        ...
        try:
            ...
            for constraint in union_constraints:
                ...
                model.addConstr(parametric_rhs_viol >= -lhs_expr + parametric_rhs, name=f'parametric_balance_power_lower_rhs_viol_{unit_id}_{time_slot}_sample_{sample_id}')
                model.addConstr(parametric_rhs_abs >= lhs_expr - parametric_rhs, name=f'parametric_balance_power_lower_rhs_abs1_{unit_id}_{time_slot}_sample_{sample_id}')
                model.addConstr(parametric_rhs_abs >= -lhs_expr + parametric_rhs, name=f'parametric_balance_power_lower_rhs_abs2_{unit_id}_{time_slot}_sample_{sample_id}')
                ...
                model.addConstr(parametric_rhs_viol >= lhs_expr - parametric_rhs, name=f'parametric_balance_power_upper_rhs_viol_{unit_id}_{time_slot}_sample_{sample_id}')
                model.addConstr(parametric_rhs_abs >= lhs_expr - parametric_rhs, name=f'parametric_balance_power_upper_rhs_abs1_{unit_id}_{time_slot}_sample_{sample_id}')
                model.addConstr(parametric_rhs_abs >= -lhs_expr + parametric_rhs, name=f'parametric_balance_power_upper_rhs_abs2_{unit_id}_{time_slot}_sample_{sample_id}')
    ...
    def _add_parametric_balance_power_penalties_pg_block_solid(self, model, x, sample_id,  zeta_values=None, union_analysis=None):
        """
        添加包含zeta参数的DCPF罚项
    ...
    def iter_with_pg_block(self, sample_id=0, theta_values=None, zeta_values=None, union_analysis=None):
        ...
        for t in range(self.T):
            ...
            # model.addConstr(power_balance_viol[t] == 0, name=f"power_balance_solid_{t}") # TODO:delete
            ...
            for g in range(self.ng):
                ...
                # model.addConstr(0 >= pg_lower_expr, name=f'pg_lower_viol_solid_{g}_{t}') ## TODO: 删除
                ...
                # model.addConstr(0 >= pg_upper_expr, name=f'pg_upper_viol_solid_{g}_{t}') ## TODO: 删除
        ...
        if self.use_zeta and union_analysis is not None and zeta_values is not None:
            ...
            obj_primal_zeta = parametric_obj_primal
            obj_opt_zeta = parametric_obj_opt
        ...
        for sample_id in range(self.n_samples):
            for g in range(self.ng):
                for t in range(self.T):
                    ...
                    model.addConstr(dual_expr_abs >= -dual_expr, name=f'dual_expr_abs_x_neg_{g}_{t}_sample_{sample_id}')
                    # 对偶约束：梯度 = 0
                    obj_dual += dual_expr_abs
    ...
    def iter(self, max_iter=20, union_analysis=None):
        ...
        for i in range(max_iter):
            ...
            # 迭代对偶块
            for sample_id in range(self.n_samples):
                lambda_sol, mu_sol, ita_lower_sol, ita_upper_sol = self.iter_with_dual_block(sample_id=sample_id, theta_values=self.theta_values, zeta_values=self.zeta_values, union_analysis=union_analysis)
    ...
    def solve_LP_with_theta_constraints(self, sample_id, theta_values, zeta_values, union_analysis=None):
        ...
        if union_analysis is not None and theta_values is not None:
            model = self._add_parametric_penalties_pg_block_solid(
                model, x, sample_id, theta_values, union_analysis
            )
    ...
    def analyse_surrogate_model_totle(self):
        ...
        print(f"最优间隙（四舍五入不含theta约束）: {differ_LP_heu}")
        print(f"最优间隙（四舍五入含theta约束）: {differ_LP_heu_refined}")    
        print(f"特殊解恢复恢复间隙: {differ_spec_heu}")
